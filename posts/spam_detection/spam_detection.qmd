...
In case of text, the preprocessing is fundamental: the computer does not understand the semantic or grammatical meaning of words.
In case of text preprocessing, we follow the following steps:
...
Using `SpaCy`, we can applied these steps. The method `nlp.pipe` improves the performance and returns a generator. It yields a `Doc` objects, not a list. To use it as a list, it has to be defined as such.
To speed up the process, is it possible to enable the multi process method in `nlp.pipe`.
But, what does the variable `nlp` stand for? It load a spaCy model: we are going to use the `en_core_web_lg`.
...
As said, the machine does not understand human readable text. It has to be transformed. The best approach is to **vectorize** it with `TfidfVectorizer()`. It is a tool for converting text into a matrix of TF-IDF features.
The **T**erm**F**requency-**I**nverse**D**ocument**F**requency is a statistical method. It is a measure of importance of a word in a document, part of a corpus, adjusted for the frequency in the corpus.
The model vectorize a word by multiplying the word **T**erm **F**requency
...
It is an Artificial Neural Network used for classification. It consists of **multiple layers of nodes**, called **perceptrons**.
For further reading, see the [documentation](https://scikit-learn.org/stable/modules/neural_networks_supervised.html).
